...
class ExecutionInfo:   
    ...
    def __init__(self,description,executeable,parameters,output,expectedThreadCount=-1,expectedRunningtime=-1,runtime='none',runtimeparameters=[]):
        ...
        self.expectedRunningtime = expectedRunningtime
        self.runtime = runtime
        self.runtimeparameters = runtimeparameters
        for i in range(0, len(self.runtimeparameters)):
            self.runtimeparameters[i] = str(self.runtimeparameters[i])
        
        # backwards compatibility (hopefully)
        if self.runtime == 'java' and self.runtimeparameters == []:
           self.runtimeparameters = ['-jar']
        if self.runtime == 'scala' and self.runtimeparameters == []:
           self.runtimeparameters = ['-classpath', '.']
    ...
    def measure(self):
        ...
        if len(sys.argv) < 2:
            Log.print("No filename for performance evaluation results given! Writing to evaluation.csv.")
        else:
            evaluationFile = sys.argv[1]
        result = self.run()
        if not result == 0:
            return 10 + result
        evaluationText = "{};{:.10f}".format(self.maxThreadCount,self.runningtime)
    ...
    def __run(self):
        ...
        if self.runtime == 'scala':
            self.executeable.ensurefileexists('.class')
        elif self.runtime == 'erl':
            self.executeable.ensurefileexists('.beam')
        elif self.runtime == 'javaOrNative':
            if self.executeable.doesnotexist('.jar'):
                self.executeable.ensurefileexists()
                self.runtime = 'none'
            else:
                self.runtime = 'java'
                self.executeable.name = self.executeable.name + '.jar'
        ...
        try:
            ...
            else: # using a runtime
                Log.print('Running:          ' +self.runtime + ' ' + ' '.join(self.runtimeparameters) + ' ' + str(self.executeable) + ' ' + ' '.join(self.parameters))
                self.__execute(self.runtime, self.runtimeparameters + [self.executeable.name] + self.parameters)
        finally:
        ...
        if self.expectedRunningtime == -1:
            Log.print("Runtime:          {:.2f}".format(self.runningtime))
        else:
            self.runtimequality = "Ok"
            if self.runningtime < 0.9 * self.expectedRunningtime:
                self.runtimequality = "Too Fast!"
            if self.runningtime > 1.1 * self.expectedRunningtime:
                self.runtimequality = "Took to Long!"
            Log.print("Runtime:          {:.2f} [{}]".format(self.runningtime,self.runtimequality))
...
class FileInfo:
    ...
    def __init__(self,name,ideal=None,idealsize=-1):
        self.name = name        
        self.ideal = None
    ...
    def doesnotexist(self,extension=''):
        name = self.name+extension
        return not os.path.exists(name) or not os.path.isfile(name)

    def isempty(self,extension=''):
        name = self.name+extension
        return os.path.getsize(name) == 0

    # check if file exists
    def ensurefileexists(self,extension=''):
        if self.doesnotexist(extension):
            raise ErrorMessage("File not found: " + os.path.realpath(self.name+extension), 1)
        if self.isempty(extension):
            raise ErrorMessage("File is empty: " + os.path.realpath(self.name+extension), 1)
    ...
    def checkfile(self):
        ...
        if self.idealsize > -1 and len(content) != self.idealsize:
            raise ErrorMessage("Result file does not have the expected file size of " + str(self.idealsize) + ".", 3)
